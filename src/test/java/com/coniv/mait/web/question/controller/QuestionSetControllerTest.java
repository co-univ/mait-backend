...
class QuestionSetControllerTest {
	...
	void createQuestionSetInvalidRequestTest(String testName, CreateQuestionSetApiRequest request,
	...
	@Test
	@DisplayName("문제 셋에서 카테고리 단건 매핑 제거 API 성공 테스트")
	void detachCategoryApiSuccess() throws Exception {
		...
		// when & then
		mockMvc.perform(delete("/api/v1/question-sets/{questionSetId}/categories/{categoryId}",
				questionSetId, categoryId))
			.andExpect(status().isOk());

		verify(questionSetCategoryService).detachCategory(questionSetId, categoryId, USER_ID);
	}
