...
public class UpdateShortQuestionApiRequest extends UpdateQuestionApiRequest {
	...
	public boolean isMainAnswerCountMatchesNumberCount() {
		...
		if (answers.stream().anyMatch(a -> a == null || a.getNumber() == null)) {
			return true;
		}

		long mainAnswerCount = answers.stream().filter(ShortAnswerDto::isMain).count();
		long numberCount = answers.stream().map(ShortAnswerDto::getNumber).distinct().count();
